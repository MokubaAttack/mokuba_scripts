...
from ..common.lyco2 import lyco2sdxl
...
class mokusdxlpipe:
	...
	def from_safe(self,path,torch_dtype=torch.float,device="cpu",vae_path="vae.safetensors"):
		...
		self.pipe=StableDiffusionXLPipeline.from_single_file(path,torch_dtype=torch_dtype)
		self.pipe.to(device)
	...
	def from_folder(self,path,torch_dtype=torch.float,device="cpu",vae_path="vae.safetensors"):
		...
		self.pipe=StableDiffusionXLPipeline.from_pretrained(path,torch_dtype=torch_dtype)
		self.pipe.to(device)
		print(path+" is loaded.")
	...
	def load_lycoris(self,path,weight=1,unet=True,teen1=True,teen2=True):
		i,w=get_id(path,weight)
		self.moku_meta["lora"] += i
		self.moku_meta["w"] += w
		
		unet_sd,text_encoder_sd,text_encoder_2_sd=lyco2sdxl(path)

		if unet_sd=={} and text_encoder_sd=={} and text_encoder_2_sd=={}:
		...
		del text_encoder_sd
		flush()
		if text_encoder_2_sd!={} and teen2:
			wrapper, _ = create_lycoris_from_weights(multiplier=weight,file="dummy.safetensors",module=self.pipe.text_encoder_2, weights_sd=text_encoder_2_sd)
			wrapper.merge_to()
			del wrapper
		del text_encoder_2_sd
		flush()
		print(path+" is loaded.")
	...
	def load_pos_embed(self,path):
		...
		self.pipe.load_textual_inversion(state_dict["clip_g"],token=key,text_encoder=self.pipe.text_encoder_2,tokenizer=self.pipe.tokenizer_2)
		self.pipe.load_textual_inversion(state_dict["clip_l"],token=key,text_encoder=self.pipe.text_encoder,tokenizer=self.pipe.tokenizer)
		del state_dict
	...
	def tileup(
		...
		if ccs==0:
			...
			self.moku_meta["cont"]=str(370104)
