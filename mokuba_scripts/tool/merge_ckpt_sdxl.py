...
from ..common.sdxldata import sdxl_data
from ..common.flush import flush
from ..common.save_safe import save_safe
from ..common.dl import dlc

def mergeckpt(ckpts,weights,v,out_path,mode="normal",dp=0,seed=0,win=None,token=""):
	if win!=None:
	...
	for k in sdxl_data:
		...
		if k in sd1:
			...
			t1=torch.zeros(sdxl_data[k]).to(torch.float32)

		if k in sd2:
			t2=sd2.pop(k)
			t2=t2.to(torch.float32)
		else:
			t2=torch.zeros(sdxl_data[k]).to(torch.float32)
	...
	if type(v) is str:
		if os.path.exists(v):
			...
			try:
				...
				if win==None:
					print(os.path.basename(v)+" does not exist.")
				else:
					win["RUN"].Update(disabled=False)
					win["info"].update(os.path.basename(v)+" does not exist.")
				if os.path.exists(os.getcwd()+"/safe_temp"):
					shutil.rmtree(os.getcwd()+"/safe_temp")
				return
			for k in vae_dict:
	...
	save_safe(list(sdxl_data),out_path,os.getcwd()+"/safe_temp")
...
def gui():
	...
	window = sg.Window('Merge Ckpt SDXL', layout)
	...
	while True:
		...
		if event == sg.WINDOW_CLOSED:
			...
			if values["out"]!="" and values["ckpt1"]!="" and values["ckpt2"]!="":
					...
					for i in range(20):
						if values["w"+str(i+1)]!="":
							try:
								...
								weights.append(r)
								window["w"+str(i+1)].update(str(weights[i]))
							except:
								if i==0:
									weights.append(0.5)
				...
				thread1 = threading.Thread(target=mergeckpt,args=(ckpts,weights,v,out_path,mode,dp,seed,window,token))
				thread1.start()
		elif event=="cancel":
			for i in range(2):
